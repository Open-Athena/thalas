...
import draccus
import fsspec
import thalas.utils as fsspec_utils
import ray
...
class Executor:
    ...
    def write_infos(self):
        ...
        for step, info in zip(self.steps, self.step_infos, strict=True):
            ...
            fsspec_utils.fsspec_mkdirs(os.path.dirname(info_path))
            with fsspec.open(info_path, "w") as f:
                print(json.dumps(asdict(info), indent=2, cls=CustomJsonEncoder), file=f)

        # Write out info for the entire execution
        fsspec_utils.fsspec_mkdirs(os.path.dirname(self.executor_info_path))
